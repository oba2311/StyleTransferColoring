# from: https://github.com/titu1994/Neural-Style-Transfer/blob/master/mask_transfer.py
...
def load_mask(mask_path, shape):
    # ‘L’ (8-bit pixels, black and white) - turn the picture into an array of balck and white pixels:
    mask = imread(mask_path, mode="L") # Grayscale mask load
    width, height, _ = shape
    # interp='bicubic', means that the interpolation is smoother than linear:
    # "In contrast to bilinear interpolation, which only takes 4 pixels (2×2) into account,
    #  bicubic interpolation considers 16 pixels (4×4). Images resampled with bicubic interpolation are smoother
...
# take the max of the array and divide mask by it + assign it to mask:
    max = np.amax(mask)
    mask /= max
...
# for pixels that are 0, i.e. balck, assign the corresponding pixels from the content image:
